...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              进化引擎 (EvolutionPlugin)                    │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────┐ │   │
│  │  │技能轨迹挖掘│→│模式提炼   │→│模板生成   │→│人工审核 │ │   │
│  │  │(SkillTrace│  │(Pattern  │  │(Template │  │(Reviewer│ │   │
│  │  │ Store)    │  │ Miner)   │  │ Generator)│  │ Approve)│ │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └────────┘ │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
                            │
...
| **DPO 训练** | 偏好反馈数据集构建、自动触发训练（阈值 2000 条） | ✅ |
...
### V0.2.0 进化系统与性能优化（已上线）

| 模块 | 功能 | 状态 |
|------|------|------|
| **进化引擎** | 技能轨迹挖掘 → 模式提炼 → 模板生成 → 人工审核 → 自动迭代全链路 | ✅ |
| **PatternMiner** | 从 skill_trace 数据挖掘高频技能序列模式（min_support + success_rate 双阈值） | ✅ |
...
│   │   ├── preference_store.py   # 偏好反馈存储 + 自动触发
│   │   └── evaluator.py          # A/B 评估器
│   ├── evolution/                # 进化引擎插件（V0.2.0）
│   │   ├── plugin.py             # 进化引擎主插件
│   │   ├── store.py              # SkillTraceStore（技能轨迹存储）
│   │   └── phase1/               # 审核工作台 API
...
│   │   │   ├── Models.jsx        # 模型管理
│   │   │   ├── Evolve.jsx        # 模型进化
│   │   │   └── EvolutionSystem.jsx # 进化系统（V0.2.0）
│   │   ├── components/           # UI 组件库
│   │   ├── App.jsx               # 布局 + 侧边栏导航
...
进入「模型进化」页面：
1. 配置训练数据阈值（默认 2000 条偏好反馈自动触发 DPO）
2. 查看审计日志 → 训练数据转换状态
3. 手动或自动启动 LoRA 微调
...
AUTO_TRIGGER_THRESHOLD=2000      # 自动触发 DPO 的偏好反馈阈值
```
